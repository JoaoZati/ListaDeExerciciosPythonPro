#%% 23 - Espaço utilizado pelos funcionarios
...
usuarios_txt = {'alexandre':       456123789,
"anderson":        1245698456,
'antonio':         123456456,
'carlos':          91257581,
'cesar':           987458,
...
megabytes_conversao = 0.0000009537
espaco_utilizado = [round(value*megabytes_conversao, 2) for value in usuarios_txt.values()]
soma_espaco = sum(espaco_utilizado) 
espaco_porc = [round(espaco/soma_espaco*100, 2) for espaco in espaco_utilizado]
...
array_usuarios = list(usuarios_txt.keys())
for i in range(len(array_usuarios)): print(f'{i+1}  {array_usuarios[i]}  \
                                           {espaco_utilizado[i]} MB   {espaco_porc[i]}%')
